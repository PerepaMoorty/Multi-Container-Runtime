 ...
 *   - bounded_buffer_push / bounded_buffer_pop (Task 3)
 *   - logging_thread consumer (Task 3)
 *   - producer_thread per container (Task 3)
 *   - child_fn clone entrypoint (Task 1)
 *   - run_supervisor event loop with UNIX socket (Task 1,2,4,6)
 *   - handle_client dispatcher (Task 2)
 *   - send_control_request client path (Task 2)
 *   - SIGCHLD / SIGTERM / SIGINT handlers (Task 2)
 *   - ioctl register/unregister helpers (Task 4)
 */
...
typedef struct container_record {
    ...
    int exit_signal;
    /*
     * stop_requested MUST be set to 1 before any signal is sent from
     * CMD_STOP.  The SIGCHLD handler uses it to distinguish a manual
     * stop (STOPPED) from a kernel hard-limit kill (KILLED).
     */
    int stop_requested;
 ...
 *
 * Blocks when the buffer is full (16 slots taken).
 * Returns 0 on success, -1 if shutdown begins while waiting.
 * Wakes the consumer after inserting.
 *
 * Race condition without synchronisation: two producers could
 * read buffer->count simultaneously, both see room, and both write
 * to the same slot, corrupting data.  The mutex prevents this.
 */
 ...
 * Returns -1 when shutdown is active AND the buffer is fully drained
 * (signals the logger thread it is safe to exit without losing data).
 */
int bounded_buffer_pop(bounded_buffer_t *buffer, log_item_t *item)
{
    ...
    /* Shutdown AND empty → consumer can exit cleanly */
...
/*
 * logging_thread — consumer thread (Task 3).
 *
 ...
 * Guarantee: no log lines are dropped — pop only returns -1 after
 ...
 *
 * Reads from the container's stdout/stderr pipe and pushes 4 KiB
 * chunks into the bounded buffer.  Exits when the pipe closes
 * (container exited).
 */
typedef struct {
    int pipe_read_fd;
...
/*
 * child_fn — runs INSIDE the new namespaces after clone() (Task 1).
 *
 * clone() without CLONE_VM gives the child a copy-on-write copy of
 * the parent's address space, so the cfg pointer is valid here even
 * though the parent frees its stack copy after clone() returns.
 */
...
/* ---------------------------------------------------------------
 * SIGCHLD handler — reaps exited children, updates metadata.
 *
...

        {
            ...
            if (c) {
                if (WIFSIGNALED(wstatus) && WTERMSIG(wstatus) == SIGKILL
                    ...
                    fprintf(stderr,
                            "[supervisor] container '%s' KILLED (hard limit)\n",
                            c->id);
                    ...
                    fprintf(stderr,
                            "[supervisor] container '%s' EXITED code=%d\n",
                            c->id, c->exit_code);
                }
...
/* ---------------------------------------------------------------
 * handle_client — processes one connected CLI client (Task 2).
 * Called inside the supervisor's accept loop.
 * --------------------------------------------------------------- */
static void handle_client(supervisor_ctx_t *ctx, int client_fd)
{
    ...
    case CMD_RUN: {
        ...
        /*
         * child_config_t on the heap.
         * clone() without CLONE_VM → COW copy, cfg valid in child_fn
         ...
         *   SIGCHLD      — parent receives SIGCHLD on child exit
         * No CLONE_VM → COW address space.
         */
        ...
        /* Close write-end in supervisor (only the child writes here) */
        close(pipefd[1]);
        ...
        pa = calloc(1, sizeof(producer_arg_t));
        if (pa) {
        ...
        /*
         * CMD_RUN: block until the container exits, update state.
         * CMD_START: return immediately (container runs in background).
         */
        if (req.kind == CMD_RUN) {
            int ws;
    ...
    case CMD_STOP: {
        ...
        pthread_mutex_lock(&ctx->metadata_lock);
        {
            ...
            /*
             * CRITICAL: set stop_requested BEFORE sending any signal.
             * The SIGCHLD handler reads this flag to classify the exit.
             */
            c->stop_requested = 1;
            pid = c->host_pid;
        }
...
/* ---------------------------------------------------------------
 * run_supervisor — Tasks 1, 2, 4, 6
 *
 * 1) Open /dev/container_monitor (graceful if not loaded)
 ...
 * 6) On shutdown: SIGTERM/SIGKILL containers, join logger, free memory
 * --------------------------------------------------------------- */
static int run_supervisor(const char *rootfs)
{
    ...
    /* Signal logger to drain and exit, then join */
...
/* ---------------------------------------------------------------
 * send_control_request — client side (Task 2).
 *
 * Connects to the supervisor's UNIX socket, sends the request
 * struct, reads back the response struct, prints the message.
 * For CMD_PS and CMD_LOGS, streams the trailing text that the
 * supervisor sends after the fixed response struct.
 * --------------------------------------------------------------- */
static int send_control_request(const control_request_t *req)
{
    ...
    /* PS and LOGS stream extra text after the fixed struct */
    if (resp.status == 0 &&
